...
<div class="content" style="width:920px; background:none; margin-left:275px; border:none; margin-bottom:30px;" >
          <div class="cLineB">
              <h4 class="left">砸蛋活动信息 (<?php echo ($count); ?>) 条<span class="FAQ">你本月有 <?php echo ($group["activitynum"]); ?> 次机会可免收活动创建费，已经使用了 <?php echo ($activitynum); ?> 次机会!</span></h4>
                 
              <div class="clr"></div>
          </div>
          <div class="cLine">
            <div class="pageNavigator left">
  <a href='<?php echo U('Zadan/add',array('token'=>$token));?>' title='新增砸金蛋活动' class='btnGrayS vm bigbtn'><img src="<?php echo RES;?>/images/img/activity-scratch-card.png" class="vm" />新增砸金蛋活动</a>
...
<TH class="norightborder">操作</TH>
              ...
              <TBODY>
                <TR></TR>
                  ...
                  <TD>砸金蛋 <?php echo ($list["id"]); ?></TD>
                   <TD class="norightborder">
				   <a href="index.php?g=User&m=Zadan&a=sn&id=<?php echo ($list["id"]); ?>&type=2">SN码管理</a> 
				   <a href="index.php?g=User&m=Zadan&a=edit&id=<?php echo ($list["id"]); ?>">编辑</a>  
				   <a href="
				   <?php if($list['status'] == 1): ?>javascript:drop_confirm('你确定要结束活动吗，结束后不可再开启本活动!', 'index.php?g=User&m=Zadan&a=setdes&id=<?php echo ($list["id"]); ?>');<?php else: ?>index.php?g=User&m=Zadan&a=setinc&id=<?php echo ($list["id"]); endif; ?>">
				   <?php if($list['status'] == 0): ?>开始<?php elseif($list['status'] == 2): else: ?>结束<?php endif; ?>				   
				   </a>
				  
				   <a href="index.php?g=User&m=Zadan&a=del&id=<?php echo ($list["id"]); ?>">删除</a></TD>
